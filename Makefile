...
CC		:= gcc
CFLAGS		:= $(CFLAGS) -Wall -fPIC `pkg-config --cflags libxml-2.0` `pkg-config --cflags hal-storage` `xmlrpc-c-config client --cflags`
LIBS		:= `pkg-config --libs libxml-2.0` `pkg-config --libs hal-storage` `xmlrpc-c-config client --ldadd` -lX11
...
remote:
	$(CC) -ggdb -Wall src/remote.c src/log.c -o src/remote -DHAVE_MAIN  `xmlrpc-c-config client --ldadd` -lX11
